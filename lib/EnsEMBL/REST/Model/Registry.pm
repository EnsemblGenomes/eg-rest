...
sub _load_registry {
  ...
  if(defined $self->eg_version() && $self->lookup_host() && defined $self->lookup_port() && defined $self->lookup_dbname() && defined $self->lookup_user()) {
    if($LOOKUP_AVAILABLE) {
...
sub _build_lookup {
  ...
  my $release      = $mdba->get_DataReleaseInfoAdaptor->fetch_by_ensembl_genomes_release($self->eg_version());
